...
function looksLikeAudio(buffer) {
  ...
  // AMR
  if (ascii(0, 6) === "#!AMR\n" || ascii(0, 9) === "#!AMR-WB\n") return true;
  // MP3 with ID3 tag or MPEG audio frame sync.
  if (ascii(0, 3) === "ID3") return true;
  if (b[0] === 0xFF && (b[1] === 0xFB || b[1] === 0xF3 || b[1] === 0xF2 || (b[1] & 0xE0) === 0xE0)) return true;
  // MP4/M4A family: ftyp at offset 4.
  if (ascii(4, 4) === "ftyp") {
    const brand = ascii(8, 4);
    if (["M4A ", "M4B ", "M4P ", "isom", "mp41", "mp42", "3gp4", "3gp5", "qt  "].includes(brand)) return true;
  }
  // WebM/Matroska (audio-only files are commonly .webm/.mka).
...
const submissionExtensions = new Set([".html", ".htm", ".css", ".js", ".mjs", ".cjs", ".ts", ".tsx", ".jsx", ".py", ".java", ".cs", ".cpp", ".c", ".h", ".json", ".xml", ".txt", ".md", ".zip"]);
...
function lessonMediaUrl(media, kind) {
  if (!media?.publicId) return media?.url || null;
  try {
    const transformation = kind === "audio"
      ? [{ quality: "auto" }]
...
async function start() {
  ...
  app.post("/api/admin/lessons", auth, admin, upload.fields([{ name: "note", maxCount: 1 }, { name: "video", maxCount: 1 }, { name: "audio", maxCount: 1 }]), async (req, res, next) => {
    try {
      ...
      if (audioFile) {
        ...
        const audioMime = String(audioFile.mimetype || "").toLowerCase();
        const isAudioMime = audioMime.startsWith("audio/");
        const isGenericBinary = ["application/octet-stream", "binary/octet-stream", "application/force-download"].includes(audioMime);
        const isRecognizedAudio = audioExtensions.has(audioExt) || isAudioMime || looksLikeAudio(audioFile.buffer);
        if (!isRecognizedAudio) {
          return res.status(400).json({ error: "The selected file could not be identified as an audio file. Please select an MP3, WAV, M4A, OGG, AAC, FLAC, OPUS, AMR, AIFF, CAF, MKA, 3GP, or WEBM audio file." });
        }
        // Some Android file managers send valid audio as application/octet-stream.
        // A valid audio signature is enough; the generic MIME is not rejected.
        void isGenericBinary;
      }
      let note = null;
      let video = null;
      let audio = null;
  ...
  app.get("/api/admin/lessons", auth, admin, async (req, res, next) => {
    try {
      ...
      for (const l of list) {
        ...
        result.push({ id: l._id.toString(), title: l.title, course_title: c?.title || "Unknown", note_path: l.note?.url || "", video_path: lessonMediaUrl(l.video, "video") || "", audio_path: lessonMediaUrl(l.audio, "audio") || "" });
      }
  ...
  async function searchAcademyMaterial(query, studentId) {
    ...
    const enrolled = enrolledCourseIds(student);
    const allCourses = enrolled.length ? await courseCollection.find({ _id: { $in: enrolled.map(x => new ObjectId(x)) }, locked: false }).toArray() : [];
    ...
    return {
      results: top.map(({ lesson, course, score }) => ({
        ...
        videoUrl: lessonMediaUrl(lesson.video, "video"),
        audioUrl: lessonMediaUrl(lesson.audio, "audio")
      })),
  ...
  app.post("/api/ai/chat", rateLimit({ windowMs: 60 * 1000, max: 20, keyPrefix: "ai-chat" }), auth, studentAccess, aiUpload.single("file"), async (req, res, next) => {
    ...
    try {
      ...
      if (req.body.lessonId) {
        ...
        lessonContext = { lessonId: lesson._id.toString(), title: lesson.title, courseTitle: course?.title || "Unknown", noteUrl: lesson.note?.url || null };
      }
  ...
  app.get("/api/courses/:id/content", auth, studentAccess, async (req, res, next) => {
    try {
      ...
      res.json({ course: { id: course._id.toString(), title: course.title, description: course.description, locked: !!course.locked }, lessons: list.map(l => ({ id: l._id.toString(), title: l.title, note_path: l.note?.url || "", video_path: lessonMediaUrl(l.video, "video") || "", audio_path: lessonMediaUrl(l.audio, "audio") || "", completed: done.has(l._id.toString()) })) });
    } catch (e) { next(e); }
  });
