...
const MEDIA_FIX_VERSION = "2026-09-20-chat-media3";
...
async function start() {
  ...
  async function streamChatAttachment(req, res, messageId, disposition = "inline") {
    ...
    // startup, seeking and partial playback. Forward the range to Cloudinary
    // instead of always downloading the entire object through Render.
    const range = req.headers.range;
    const upstreamHeaders = {};
    if (range && attachment.kind !== "document") upstreamHeaders.Range = range;

    const upstream = await fetch(attachment.url, { headers: upstreamHeaders, redirect: "follow" });
    ...
    const mime = attachment.kind === "document"
      ...
      : (attachment.mime && !attachment.mime.endsWith("/*") ? attachment.mime : (fallbackMime[attachment.kind]?.[ext] || upstreamType || (attachment.kind === "audio" ? "audio/wav" : attachment.kind === "video" ? "video/mp4" : "application/octet-stream")));
