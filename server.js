...
async function start() {
  ...
  function effectiveChatAttachmentKind(attachment) {
    ...
    if (audioExtensions.has(ext) && !chatVideoExtensions.has(ext)) return "audio";
    if (chatVideoExtensions.has(ext) && !audioExtensions.has(ext)) return "video";
    // .webm is valid for both audio and video. If MIME is unavailable, keep
    // the persisted kind rather than guessing from the container alone.
  ...
  function chatAttachmentInfo(file) {
    ...
    // MIME type wins for ambiguous containers such as .webm. Android Chrome
    // records voice notes as audio/webm; checking the video extension first
    // was the reason those voice notes were previously rendered as video.
    if (mime.startsWith("audio/") || audioExtensions.has(ext) || looksLikeAudio(file.buffer)) return { kind: "audio", resourceType: "video", mime: mime.startsWith("audio/") ? mime : "audio/*", ext };
