...
async function start() {
  ...
  app.get("/api/chat/conversations/:id/messages", auth, studentAccess, async (req, res, next) => {
    try {
      ...
      res.json({ messages: rows.map(m => ({ id: m._id.toString(), senderId: String(m.senderId), senderName: senderMap.get(String(m.senderId))?.name || "WPS Academy User", senderRole: senderMap.get(String(m.senderId))?.role || "student", text: m.text || "", createdAt: m.createdAt, read: (m.readBy || []).some(id => String(id) === String(req.user._id)) || String(m.senderId) === String(req.user._id) })) });
    } catch (e) { next(e); }
  });
  ...
  app.post("/api/chat/conversations/:id/messages", rateLimit({ windowMs: 10 * 1000, max: 30, keyPrefix: "chat-message" }), auth, studentAccess, async (req, res, next) => {
    try {
      ...
      if (!(await chatCanAccessConversation(req.user, conversation))) return res.status(403).json({ error: "You do not have access to this chat." });
      const now = new Date();
      const doc = { conversationId, senderId: req.user._id, text, readBy: [req.user._id], createdAt: now };
      const result = await chatMessages.insertOne(doc);
      await chatConversations.updateOne({ _id: conversationId }, { $set: { lastMessageAt: now, lastMessagePreview: text.slice(0, 120), updatedAt: now } });
      res.json({ ok: true, message: { id: result.insertedId.toString(), senderId: req.user._id.toString(), senderName: req.user.name, senderRole: req.user.role, text, createdAt: now, read: true } });
    } catch (e) { next(e); }
  });
