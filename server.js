...
const MEDIA_FIX_VERSION = "2026-09-17-chat1";
...
async function start() {
  ...
  app.get("/api/chat/conversations/:id/messages", auth, studentAccess, async (req, res, next) => {
    try {
      ...
      res.json({ messages: rows.map(m => ({ id: m._id.toString(), senderId: String(m.senderId), senderName: senderMap.get(String(m.senderId))?.name || "WPS Academy User", senderRole: senderMap.get(String(m.senderId))?.role || "student", text: m.text || "", attachment: m.attachment || null, createdAt: m.createdAt, read: (m.readBy || []).some(id => String(id) === String(req.user._id)) || String(m.senderId) === String(req.user._id) })) });
    } catch (e) { next(e); }
  });
  ...
  app.post("/api/chat/conversations/:id/messages", rateLimit({ windowMs: 10 * 1000, max: 30, keyPrefix: "chat-message" }), auth, studentAccess, async (req, res, next) => {
    try {
      ...
      if (!(await chatCanAccessConversation(req.user, conversation))) return res.status(403).json({ error: "You do not have access to this chat." });
      const now = new Date();
      const doc = { conversationId, senderId: req.user._id, text, attachment: null, readBy: [req.user._id], createdAt: now };
      ...
      res.json({ ok: true, message: { id: result.insertedId.toString(), senderId: req.user._id.toString(), senderName: req.user.name, senderRole: req.user.role, text, attachment: null, createdAt: now, read: true } });
    } catch (e) { next(e); }
  });
  ...
  app.post("/api/chat/conversations/:id/attachments", rateLimit({ windowMs: 60 * 1000, max: 20, keyPrefix: "chat-attachment" }), auth, studentAccess, upload.single("file"), async (req, res, next) => {
    try {
      ...
      const attachment = { kind: info.kind, name: safeName, mime: info.mime, size: req.file.size, url: uploaded.secure_url || uploaded.url, publicId: uploaded.public_id, resourceType: uploaded.resource_type || info.resourceType, createdAt: new Date() };
      const now = new Date();
      const doc = { conversationId, senderId: req.user._id, text: cleanChatText(req.body?.text), attachment, readBy: [req.user._id], createdAt: now };
      const result = await chatMessages.insertOne(doc);
      ...
      res.json({ ok: true, message: { id: result.insertedId.toString(), senderId: req.user._id.toString(), senderName: req.user.name, senderRole: req.user.role, text: doc.text, attachment, createdAt: now, read: true } });
    } catch (e) { next(e); }
  });

  app.get("/api/chat/attachments/:messageId/download", auth, studentAccess, async (req, res, next) => {
    try {
      const messageId = oid(req.params.messageId);
      if (!messageId) return res.status(400).json({ error: "Invalid attachment." });
      const message = await chatMessages.findOne({ _id: messageId });
      ...
      if (!upstream.ok || !upstream.body) return res.status(502).json({ error: "The attachment could not be retrieved." });
      const mime = attachment.kind === "document" ? "application/vnd.openxmlformats-officedocument.wordprocessingml.document" : (attachment.mime || upstream.headers.get("content-type") || "application/octet-stream");
      res.status(200);
      res.setHeader("Content-Type", mime);
      res.setHeader("Content-Disposition", `attachment; filename="${String(attachment.name || "attachment").replace(/[\"\r\n]/g, "_")}"`);
      if (attachment.size) res.setHeader("Content-Length", String(attachment.size));
