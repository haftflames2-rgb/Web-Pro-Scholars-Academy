...
const MEDIA_FIX_VERSION = "2026-09-22-chat-voice-fast1";
...
async function start() {
  ...
  function effectiveChatAttachmentKind(attachment) {
    ...
    if (attachment.isVoice === true) return "audio";
  ...
  function chatAttachmentInfo(file, isVoice = false) {
    if (!file) return null;
    const ext = path.extname(file.originalname || "").toLowerCase();
    const mime = String(file.mimetype || "").toLowerCase();
    // Voice recordings are explicitly marked by the chat client. This avoids
    // the WebM audio/video ambiguity while leaving ordinary WebM video uploads untouched.
    if (isVoice && (mime.startsWith("audio/") || ext === ".webm" || ext === ".ogg" || ext === ".m4a")) {
      return { kind: "audio", resourceType: "video", mime: mime.startsWith("audio/") ? mime : "audio/webm", ext, isVoice: true };
    }
  ...
  app.post("/api/chat/conversations/:id/attachments", rateLimit({ windowMs: 60 * 1000, max: 20, keyPrefix: "chat-attachment" }), auth, studentAccess, upload.single("file"), async (req, res, next) => {
    try {
      ...
      const info = chatAttachmentInfo(req.file, String(req.body?.isVoice || '') === '1');
      ...
      const attachment = { kind: info.kind, name: safeName, mime: info.mime, size: req.file.size, url: uploaded.secure_url || uploaded.url, playbackUrl: eagerPlayback, publicId: uploaded.public_id, resourceType: uploaded.resource_type || info.resourceType, isVoice: !!info.isVoice, createdAt: new Date() };
      const now = new Date();
  ...
  async function streamChatAttachment(req, res, messageId, disposition = "inline", forceOriginalMedia = false) {
    ...
    // WPS voice notes are recorded as compact WebM/Opus audio. Keep the original
    // delivery URL so Cloudinary does not have to generate an MP3 derivative
    // before the phone can start playback.
    const isWpsVoiceNote = effectiveKind === "audio" && (attachment.isVoice === true || /^voice-message-/i.test(String(attachment.name || "")));
    if (!forceOriginalMedia && disposition === "inline" && (effectiveKind === "video" || effectiveKind === "audio") && !isWpsVoiceNote) {
    ...
    if (!forceOriginalMedia && disposition === "inline" && (effectiveKind === "video" || effectiveKind === "audio") && mediaUrl) {
