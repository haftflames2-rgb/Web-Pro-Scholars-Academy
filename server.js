...
const MEDIA_FIX_VERSION = "2026-09-14-mediafix3";
...
async function start() {
  ...
  app.post("/api/admin/lessons", auth, admin, upload.fields([{ name: "note", maxCount: 1 }, { name: "video", maxCount: 1 }, { name: "audio", maxCount: 1 }]), async (req, res, next) => {
    try {
      ...
      if (audioFile && !audioUploadLooksAllowed(audioFile)) {
        return res.status(400).json({ error: "Unsupported audio file. Please choose an MP3, WAV, M4A, OGG, AAC, FLAC, OPUS, AMR, AIFF, CAF, MKA, 3GP, or WEBM audio file." });
      }
      let note = null;
      let video = null;
      let audio = null;
