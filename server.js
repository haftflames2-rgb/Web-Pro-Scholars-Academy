...
const MEDIA_FIX_VERSION = "2026-09-20-chat-media5";
...
async function start() {
  ...
  function chatAttachmentInfo(file) {
    ...
    // MediaRecorder on Android commonly produces audio/webm. Check the
    // browser-reported MIME type before the shared .webm extension, otherwise
    // a voice recording is incorrectly stored/rendered as a video.
    if (mime.startsWith("audio/") || audioExtensions.has(ext) && ext !== ".webm") return { kind: "audio", resourceType: "video", mime: mime.startsWith("audio/") ? mime : "audio/*", ext };
    if (mime.startsWith("video/") || chatVideoExtensions.has(ext)) return { kind: "video", resourceType: "video", mime: mime.startsWith("video/") ? mime : "video/*", ext };
  ...
  app.post("/api/chat/conversations/:id/attachments", rateLimit({ windowMs: 60 * 1000, max: 20, keyPrefix: "chat-attachment" }), auth, studentAccess, upload.single("file"), async (req, res, next) => {
    try {
      ...
      const attachment = { kind: info.kind, name: safeName, mime: info.mime, size: req.file.size, url: uploaded.secure_url || uploaded.url, publicId: uploaded.public_id, resourceType: uploaded.resource_type || info.resourceType, type: uploaded.type || "upload", createdAt: new Date() };
      const now = new Date();
  ...
  async function streamChatAttachment(req, res, messageId, disposition = "inline") {
    ...
    // IMPORTANT: let Cloudinary serve transformed media directly. The previous
    // implementation fetched the transformed stream through Node and copied
    // Cloudinary's byte-range headers to Express. On some Android/Cloudinary
    // combinations that makes the browser believe the response is only a
    // tiny partial file, so playback stops after ~1–2 seconds. A direct
    // redirect preserves Cloudinary's native Range/206 handling and allows
    // seeking through the complete asset.
    let playbackUrl = attachment.url;
    if (attachment.kind === "video" && attachment.publicId) {
      playbackUrl = cloudinary.url(attachment.publicId, {
        secure: true,
        resource_type: "video",
    ...
    } else if (attachment.kind === "audio" && attachment.publicId) {
      playbackUrl = cloudinary.url(attachment.publicId, {
        secure: true,
        resource_type: "video",
        type: attachment.type || "upload",
    ...
    if (attachment.kind === "video" || attachment.kind === "audio" || attachment.kind === "image") {
      return res.redirect(302, playbackUrl);
    }

    // Documents still go through the authenticated server response so the
    // browser gets the intended DOCX download headers.
    const upstream = await fetch(playbackUrl, { redirect: "follow" });
