...
const MEDIA_FIX_VERSION = "2026-09-16-livefix3";
...
const submissionExtensions = new Set([".html", ".htm", ".css", ".js", ".mjs", ".cjs", ".ts", ".tsx", ".jsx", ".py", ".java", ".cs", ".cpp", ".c", ".h", ".json", ".xml", ".txt", ".md", ".zip", ".doc", ".docx", ".pdf"]);
...
async function start() {
  ...
  // Proxy student submission downloads through the LMS instead of linking
  // directly to Cloudinary. Cloudinary raw assets may be returned as
  // application/octet-stream, which can make DOCX files download as .bin on
  // mobile browsers. We restore the original filename and MIME type here.
  app.get("/api/admin/submissions/:id/file", auth, admin, async (req, res, next) => {
    try {
      ...
      if (!upstream.ok) {
        console.error(`Submission file upstream returned ${upstream.status} for submission ${id}`);
        return res.status(upstream.status === 404 ? 404 : 502).json({ error: "Submission file could not be loaded." });
      }

      const filename = path.basename(submission.file.originalName || "submission") || "submission";
