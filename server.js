...
async function start() {
  ...
  app.get("/api/public-courses", async (req, res, next) => {
    try {
      ...
      res.json({ courses: list.map(c => ({ id: c._id.toString(), title: c.title, description: c.description })) });
    } catch (e) { next(e); }
  });
  ...
  app.get("/api/courses", async (req, res, next) => {
    try {
      ...
      res.json({ courses: filtered.map(c => ({ id: c._id.toString(), title: c.title, description: c.description, locked: !!c.locked, registered: viewer?.role === "admin" ? true : !!viewer && studentHasCourse(viewer, c._id) })) });
    } catch (e) { next(e); }
  });
