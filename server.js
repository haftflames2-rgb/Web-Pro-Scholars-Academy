...
async function start() {
  ...
  app.delete("/api/admin/users/:id", auth, admin, async (req, res, next) => {
    try {
      ...
      if (req.user._id.equals(id)) return res.status(400).json({ error: "You cannot delete the currently signed-in admin account." });
      const user = await users.findOne({ _id: id });
      if (!user) return res.status(404).json({ error: "User not found" });
      if (user.role === "admin") return res.status(400).json({ error: "Admin accounts cannot be deleted from the student manager." });

      const [userPayments, userSubmissions, userArtifacts] = await Promise.all([
        payments.find({ userId: id }).toArray(),
        submissions.find({ studentId: id }).toArray(),
        aiArtifacts.find({ userId: id }).toArray()
      ]);

      const assets = [];
      for (const payment of userPayments) if (payment.proof?.publicId) assets.push(destroyCloudinaryAsset(payment.proof, payment.proof.resourceType || "image"));
      for (const submission of userSubmissions) if (submission.file?.publicId) assets.push(destroyCloudinaryAsset(submission.file, "raw"));
      for (const artifact of userArtifacts) if (artifact.publicId) {
        const artifactResourceType = artifact.resourceType || (artifact.type === "video" ? "video" : artifact.type === "image" ? "image" : "raw");
        assets.push(destroyCloudinaryAsset({ publicId: artifact.publicId, resourceType: artifactResourceType, type: "upload" }, artifactResourceType));
      }
      ...
      await Promise.all([
        ...
        db.collection("sessions").deleteMany({ "session.userId": id.toString() }),
