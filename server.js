...
async function start() {
  ...
  async function startAIVideo(prompt, userId) {
    ...
    await aiArtifacts.insertOne({ userId, type: "video", jobId: data.id, title: String(prompt).slice(0, 100), status: data.status || "queued", progress: data.progress || 0, createdAt: new Date(), updatedAt: new Date() });
  ...
  app.post("/api/ai/quiz", auth, studentAccess, async (req, res, next) => {
    ...
    } catch (e) { console.error('Quiz error:', e); next(e); }
  });
  ...
  app.get("/api/ai/status", auth, studentAccess, (req, res) => {
    res.json({ enabled: aiEnabled, model: AI_MODEL, webSearch: String(process.env.OPENAI_ENABLE_WEB_SEARCH || "false").toLowerCase() === "true" });
  });
  ...
  app.post("/api/ai/speech", auth, studentAccess, async (req, res, next) => {
    try {
      ...
      const response = await fetch("https://api.openai.com/v1/audio/speech", {
        ...
        body: JSON.stringify({ model: process.env.OPENAI_TTS_MODEL || "gpt-4o-mini-tts", voice: process.env.OPENAI_TTS_VOICE || "marin", input: text, response_format: "mp3" })
      });
  ...
  app.post("/api/ai/video", auth, studentAccess, async (req, res, next) => {
    try {
      ...
      res.json(await startAIVideo(prompt, req.user._id));
    } catch (e) {
    ...
    try {
      ...
      const result = await createTutorResponse({
        ...
        toolExecutor: async (name, args) => {
          ...
          if (name === "generate_image") return generateAIImage(args.prompt, req.user._id);
          if (name === "generate_document") return generateAIDocument(args.title, args.content, args.format, req.user._id);
          if (name === "generate_video") return startAIVideo(args.prompt, req.user._id);
      ...
      if (e?.status === 401) return res.status(502).json({ error: "The AI service rejected the API key. Check OPENAI_API_KEY on Render." });
      if (e?.status === 429) return res.status(429).json({ error: "The AI service is temporarily busy or your API limit was reached. Please try again shortly." });
      if (e instanceof multer.MulterError && e.code === "LIMIT_FILE_SIZE") return res.status(413).json({ error: "AI attachments must be 15 MB or smaller." });
      next(e);
    }
  });
