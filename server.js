...
const MEDIA_FIX_VERSION = "2026-09-21-chat-media6-range-stream";
...
async function start() {
  ...
  async function streamChatAttachment(req, res, messageId, disposition = "inline") {
    ...
    let upstream;
    if (req.method === "HEAD") {
      upstream = await fetch(attachment.url, { method: "HEAD", headers: upstreamHeaders, redirect: "follow" });
    } else {
      upstream = await fetch(attachment.url, { headers: upstreamHeaders, redirect: "follow" });
    }
    ...
    const mime = attachment.kind === "document" ? upstreamTypeHint : (upstreamType && !/^application\/octet-stream(?:;|$)/i.test(upstreamType) ? upstreamType : upstreamTypeHint);
