...
const audioExtensions = new Set([".mp3", ".wav", ".m4a", ".ogg", ".aac", ".flac", ".opus", ".amr", ".webm"]);
  ...
  app.post("/api/admin/lessons", auth, admin, upload.fields([{ name: "note", maxCount: 1 }, { name: "video", maxCount: 1 }, { name: "audio", maxCount: 1 }]), async (req, res, next) => {
    try {
      ...
      if (audioFile) {
        ...
        const isAudioMime = String(audioFile.mimetype || "").toLowerCase().startsWith("audio/");
        if (!audioExtensions.has(audioExt) && !isAudioMime) {
          return res.status(400).json({ error: "The selected file is not recognized as an audio file. Please choose a supported audio file." });
        }
      }
      let note = null;
      let video = null;
