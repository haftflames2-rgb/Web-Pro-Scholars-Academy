...
const MEDIA_FIX_VERSION = "2026-09-16-aifix2-fallback";
...
async function start() {
  ...
  app.post("/api/ai/speech", rateLimit({ windowMs: 60 * 1000, max: 10, keyPrefix: "ai-speech" }), auth, studentAccess, async (req, res, next) => {
    ...
    try {
      if (!process.env.OPENAI_API_KEY) {
        return res.status(503).json({ error: "AI voice is not configured on the server.", code: "AI_NOT_CONFIGURED" });
      }
      ...
      quota = await requireAICredits(req, res, "speech");
      if (!quota) return;
      ...
      if (!response.ok) {
        ...
        if (response.status === 429 || /credit|quota|billing|no credits|rate limit/i.test(upstreamMessage)) {
          const err = new Error("OpenAI voice is unavailable because the API account has no available credits or has reached its limit. Add API credits or check the API project limits.");
          err.status = 402; err.code = "OPENAI_CREDITS_OR_LIMIT"; err.upstreamCode = upstreamCode;
          throw err;
        }
        const err = new Error(upstreamMessage.slice(0, 500));
        err.status = response.status >= 500 ? 502 : 400;
        err.code = "OPENAI_SPEECH_ERROR";
      ...
      if (e?.name === "AbortError") {
        return res.status(504).json({ error: "AI voice took too long to respond. Please try the speaker button again.", code: "OPENAI_SPEECH_TIMEOUT" });
      }
      if (e?.status && e?.code) {
  ...
  app.get("/api/courses/:id/content", auth, studentAccess, async (req, res, next) => {
    try {
      ...
      res.json({ course: { id: course._id.toString(), title: course.title, description: course.description, locked: !!course.locked }, lessons: list.map(l => ({ id: l._id.toString(), title: l.title, note_path: l.note?.url || "", video_path: l.video?.url ? `/api/learning/lessons/${l._id}/media/video` : "", audio_path: l.audio?.url ? `/api/learning/lessons/${l._id}/media/audio` : "", completed: done.has(l._id.toString()) })) });
    } catch (e) { next(e); }
  });
