...
const MEDIA_FIX_VERSION = "2026-09-20-chat-media4";
...
async function start() {
  ...
  async function streamChatAttachment(req, res, messageId, disposition = "inline") {
    ...
    // Use a browser-friendly Cloudinary delivery format for chat playback.
    // Chat uploads can be WebM/WAV/AMR/etc. and some Android browsers stop
    // playback after a short initial chunk when those source formats are
    // streamed directly. Cloudinary can transcode the delivery representation
    // while keeping the original uploaded asset unchanged.
    ...
    // Media elements on Android/iOS rely on HTTP Range requests for fast
    // startup, seeking and full playback. Forward the browser's Range header
    // to Cloudinary and preserve Cloudinary's range metadata on the response.
    const range = req.headers.range;
    const upstreamHeaders = {};
    if (range && attachment.kind !== "document") upstreamHeaders.Range = range;

    const upstream = await fetch(playbackUrl, { headers: upstreamHeaders, redirect: "follow" });
    ...
    const fallbackMime = {
      ...
      video: { ".mp4": "video/mp4", ".webm": "video/webm", ".mov": "video/quicktime", ".m4v": "video/x-m4v", ".3gp": "video/3gpp" },
      audio: { ".wav": "audio/wav", ".mp3": "audio/mpeg", ".m4a": "audio/mp4", ".ogg": "audio/ogg", ".oga": "audio/ogg", ".opus": "audio/ogg", ".aac": "audio/aac", ".flac": "audio/flac", ".webm": "audio/webm", ".amr": "audio/amr", ".aif": "audio/aiff", ".aiff": "audio/aiff", ".caf": "audio/x-caf" }
    };
    const upstreamType = upstream.headers.get("content-type") || "";
    const mime = attachment.kind === "document"
      ? "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
      : attachment.kind === "video"
        ? "video/mp4"
        : attachment.kind === "audio"
          ? "audio/mpeg"
          : (attachment.mime && !attachment.mime.endsWith("/*") ? attachment.mime : (fallbackMime[attachment.kind]?.[ext] || upstreamType || "application/octet-stream"));
