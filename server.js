...
const audioExtensions = new Set([
  ".mp3", ".wav", ".m4a", ".ogg", ".aac", ".flac", ".opus", ".amr", ".webm"
]);
...
function isRecognizedAudioFile(file) {
  ...
  if (audioExtensions.has(ext) || mime.startsWith("audio/")) return true;

  const b = file.buffer;
  if (!Buffer.isBuffer(b) || b.length < 4) return false;

  // Recognize common audio containers/codecs even when the filename has no extension.
  if (b.subarray(0, 4).toString("ascii") === "RIFF" && b.subarray(8, 12).toString("ascii") === "WAVE") return true; // WAV
  if (b.subarray(0, 4).toString("ascii") === "OggS") return true; // OGG/OPUS
  if (b.subarray(0, 4).toString("ascii") === "fLaC") return true; // FLAC
  if (b.subarray(0, 6).toString("ascii") === "ID3") return true; // MP3 with ID3 tag
  if (b[0] === 0x23 && b.subarray(0, 6).toString("ascii") === "#!AMR") return true; // AMR
  if (b[0] === 0x1a && b[1] === 0x45 && b[2] === 0xdf && b[3] === 0xa3) return true; // WebM/Matroska
  if ((b[0] === 0xff && (b[1] & 0xe0) === 0xe0) || (b[0] === 0xff && (b[1] & 0xf6) === 0xf0)) return true; // MPEG/AAC frame sync

  // M4A/MP4 audio files use an ISO Base Media File Format "ftyp" box.
  if (b.length >= 12 && b.subarray(4, 8).toString("ascii") === "ftyp") {
    const brand = b.subarray(8, 12).toString("ascii").toLowerCase();
    if (["m4a ", "m4b ", "m4p ", "mp4 ", "isom", "iso2", "mp41", "mp42", "qt  "].includes(brand)) return true;
  }
...
async function start() {
  ...
  app.post("/api/admin/lessons", auth, admin, upload.fields([{ name: "note", maxCount: 1 }, { name: "video", maxCount: 1 }, { name: "audio", maxCount: 1 }]), async (req, res, next) => {
    try {
      ...
      if (audioFile && !isRecognizedAudioFile(audioFile)) {
        return res.status(400).json({
          error: "The selected file is not recognized as an audio file. Please choose an MP3, WAV, M4A, OGG, AAC, FLAC, OPUS, AMR, or WebM audio file."
        });
      }
      let note = null;
      let video = null;
  ...
  app.get("/api/admin/lessons", auth, admin, async (req, res, next) => {
    try {
      ...
      for (const l of list) {
        ...
        result.push({ id: l._id.toString(), title: l.title, course_title: c?.title || "Unknown", note_path: l.note?.url || "", video_path: l.video?.url || "", audio_path: l.audio?.url || "" });
      }
