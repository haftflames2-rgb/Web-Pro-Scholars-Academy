...
### Voice profile
WPS Academy is configured to use the `coral` OpenAI TTS voice by default, with instructions for a light, warm, friendly English tutoring voice. If `OPENAI_TTS_VOICE` is set in Render, that environment variable takes priority over the default.
