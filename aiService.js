...
const SAFE_OPENAI_MODEL = "gpt-5.6-luna";
const configuredOpenAIModel = String(process.env.OPENAI_MODEL || "").trim();
// Older WPS builds used the non-existent `gpt-6-astra` alias. Keep deployments
// that still have that old Render variable working by transparently using a
// currently supported Responses API model.
const MODEL = (!configuredOpenAIModel || configuredOpenAIModel === "gpt-6-astra")
  ? SAFE_OPENAI_MODEL
...
async function runResponses({ primary, user, input, useWeb, toolsForRequest, context, modelOverride = null }) {
  ...
  return providerClient.responses.create({
    model: modelOverride || getProviderModel(primary),
...
function isModelCompatibilityError(error) {
  ...
  const code = String(error?.code || error?.error?.code || "").toLowerCase();
  return status === 400 || status === 404 || code === "model_not_found" || /model.*(not found|does not exist|not available|unsupported)|unknown model/.test(msg);
}
...
async function createTutorResponse({ user, messages, toolExecutor, attachedFiles = [], lessonContext = null, academyContext = null, useWeb = false }) {
  ...
  try {
    ...
    // If an old Render variable still points at a retired/invalid model,
    // retry once with the current supported WPS default before using backup AI.
    if (primary && MODEL !== SAFE_OPENAI_MODEL && isModelCompatibilityError(error)) {
      console.warn(`Configured OpenAI model "${MODEL}" was rejected; retrying with ${SAFE_OPENAI_MODEL}.`);
      try {
        response = await runResponses({ primary, user, input, useWeb, toolsForRequest: tools, context, modelOverride: SAFE_OPENAI_MODEL });
      } catch (retryError) {
        error = retryError;
      }
    }
  ...
  for (let round = 0; round < 4; round++) {
    ...
    try {
      ...
      if (primary && MODEL !== SAFE_OPENAI_MODEL && isModelCompatibilityError(error)) {
        console.warn(`Configured OpenAI model "${MODEL}" was rejected during tool execution; retrying with ${SAFE_OPENAI_MODEL}.`);
        try {
          response = await runResponses({ primary, user, input: nextInput, useWeb: false, toolsForRequest: tools, context, modelOverride: SAFE_OPENAI_MODEL });
        } catch (retryError) {
          error = retryError;
        }
      }
...
async function generateTutorText({ user, prompt }) {
  ...
  try {
    ...
    if (primary && MODEL !== SAFE_OPENAI_MODEL && isModelCompatibilityError(error)) {
      console.warn(`Configured OpenAI model "${MODEL}" was rejected for text generation; retrying with ${SAFE_OPENAI_MODEL}.`);
      try {
    ...
    if (!response) {
      ...
      const messages = chatMessagesFromInput(backupInputForFiles(input), context);
      const completion = await backupClient.chat.completions.create({ model: BACKUP_MODEL, messages, stream: false });
      const message = completion.choices?.[0]?.message;
      const text = String(message?.content || "").trim();
      if (!text) throw new Error("Hugging Face returned an empty response.");
      return { text, responseId: completion.id, provider: "huggingface", model: BACKUP_MODEL };
