...
async function runBackupChat({ input, context, toolExecutor }) {
  ...
  for (let round = 0; round < 4; round++) {
    const completion = await backupClient.chat.completions.create({
      model: BACKUP_MODEL,
      messages,
      tools: backupChatTools(),
      tool_choice: "auto"
    });
    const message = completion.choices?.[0]?.message;
    if (!message) throw new Error("Hugging Face returned an empty response.");
    if (!message.tool_calls?.length) {
      return { text: message.content || "I couldn't generate a response.", responseId: completion.id, provider: "huggingface", model: BACKUP_MODEL };
    }
    ...
    for (const call of message.tool_calls) {
      ...
      let result;
      try { result = await toolExecutor(call.function?.name, args); }
      catch (err) { result = { error: err.message || "Tool failed" }; }
      messages.push({ role: "tool", tool_call_id: call.id, content: JSON.stringify(result) });
    }
  }
  throw new Error("Hugging Face backup reached the tool-call limit without producing an answer.");
}
