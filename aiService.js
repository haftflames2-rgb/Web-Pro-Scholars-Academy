...
function shouldTryBackup(error) {
  ...
  const unsupportedFileType = status === 400 && (message.includes("file type") || message.includes("unsupported") || message.includes("input_file"));
  return backupEnabled && (status === 401 || status === 403 || status === 408 || status === 409 || status === 429 || status >= 500 || unsupportedFileType || ["insufficient_quota", "credit_balance_exhausted", "rate_limit_exceeded", "api_connection_error", "timeout"].includes(code));
}
...
async function createTutorResponse({ user, messages, toolExecutor, attachedFiles = [], lessonContext = null, academyContext = null, useWeb = false }) {
  ...
  // Do not automatically send Cloudinary lesson-note URLs as OpenAI input_file
  // parts. Some Cloudinary delivery URLs do not expose a filename/extension in
  // the form OpenAI's file validator expects, which causes HTTP 400
  // "file type ... not supported" errors for otherwise normal student questions.
  // The academy metadata is still passed as text below, and the backup provider
  // intentionally strips file parts as well. Explicit files uploaded by the user
  // remain supported through attachedFiles.
