...
async function init(){
 try{
  ...
  document.getElementById('assignments').innerHTML=d.assignments.length?d.assignments.map(a=>`
   ...
   <form class="submitForm" data-id="${a.id}" enctype="multipart/form-data"><textarea name="textCode" placeholder="Paste your code here (optional when uploading a file)"></textarea><input type="file" name="codeFile" accept=".zip,.doc,.docx,.pdf,.js,.ts,.py,.html,.css,.jsx,.tsx,.cs,.java,.txt,.md,.json,.xml"><button class="btn">Submit Assignment</button></form></div>`).join(''):'<p>No assignments yet.</p>';
