...
async function init(){
 try{
  ...
  async function openCourse(id){const target=document.getElementById('course-'+id);try{const x=await api('/api/courses/'+id+'/content');target.innerHTML=x.lessons.length?x.lessons.map(l=>`<div class="panel lesson-card"><div class="lesson-head"><h4>${esc(l.title)}</h4><button class="btn small lesson-complete" data-id="${l.id}" data-completed="${l.completed?'true':'false'}">${l.completed?'✓ Completed':'Mark complete'}</button></div>${l.note_path?`<p><a href="${l.note_path}" target="_blank" rel="noopener">📄 Open lecture note</a> <a class="btn small" href="/ai-tutor.html?lessonId=${encodeURIComponent(l.id)}">🤖 Explain with AI</a></p>`:`<p><a class="btn small" href="/ai-tutor.html?lessonId=${encodeURIComponent(l.id)}">🤖 Ask AI about lesson</a></p>`}${l.video_path?`<video controls preload="metadata" playsinline style="width:100%;border-radius:10px"><source src="${l.video_path}">Your browser could not play this lesson video.</video>`:''}${l.audio_path?`<div style="margin-top:12px"><p><b>🎵 Lesson audio</b></p><audio controls preload="metadata" style="width:100%"><source src="${l.audio_path}">Your browser could not play this lesson audio.</audio></div>`:''}</div>`).join(''):'<p>No lessons uploaded yet.</p>';document.querySelectorAll('.lesson-complete').forEach(btn=>btn.onclick=async()=>{const completed=btn.dataset.completed!=='true';try{await api('/api/learning/lessons/'+btn.dataset.id+'/progress',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({completed})});btn.dataset.completed=String(completed);btn.textContent=completed?'✓ Completed':'Mark complete';loadLearningSummary();}catch(e){alert(e.message)}});}catch(e){target.innerHTML='<p>'+esc(e.message)+'</p>'}}
