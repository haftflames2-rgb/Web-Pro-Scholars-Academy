...
async function init(){
 try{
  ...
  document.getElementById('assignments').innerHTML=d.assignments.length?d.assignments.map(a=>`
   ...
   <form class="submitForm" data-id="${a.id}" enctype="multipart/form-data"><textarea name="textCode" placeholder="Paste your code here"></textarea><input type="file" name="codeFile" accept=".zip,.js,.ts,.py,.html,.css,.jsx,.tsx,.cs,.java,.txt,.doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"><button class="btn">Submit Assignment</button></form></div>`).join(''):'<p>No assignments yet.</p>';
