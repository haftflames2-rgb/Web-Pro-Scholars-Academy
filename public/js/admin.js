...
async function init() {
  try {
    ...
    function renderUsers() {
      ...
      const list = (window.__adminUsers || []).filter(x => x.role === 'student' && (!query || `${x.name || ''} ${x.email || ''}`.toLowerCase().includes(query)));
      const meta = document.getElementById('studentSearchMeta');
      if (meta) meta.textContent = query ? `${list.length} student${list.length === 1 ? '' : 's'} found.` : `${list.length} student${list.length === 1 ? '' : 's'} total.`;
      document.getElementById('users').innerHTML = list.map(x => {
        ...
        return `<div class="row" style="display:block">
          <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap">
            ...
            <span>
              ...
              <button class="ghost-btn small admin-action" data-action="delete-user" data-id="${x.id}">Delete User</button>
            </span>
          </div>
    ...
    document.getElementById('studentSearch').addEventListener('input', renderUsers);
    ...
    async function runAdminAction(btn, refreshFn) {
      ...
      try {
        if (action === 'approve') {
          ...
          const user = (window.__adminUsers || []).find(x => x.id === id);
          if (!confirm(`Delete ${user?.name || 'this user'}? This permanently deletes the account, payments, submissions, AI history, progress, and associated Cloudinary files. This cannot be undone.`)) {
    ...
    try {
      ...
      warning.textContent = 'Some admin data could not be loaded. Your admin session is still active; please refresh or try again. ' + (err.message || '');
      document.querySelector('main.section')?.prepend(warning);
    }
  } catch (err) {
    ...
    if (err.status === 401 || err.status === 403) {
      ...
      alert('The admin portal could not connect to the server. Please check your Render deployment and try again.');
    }
  }
}
