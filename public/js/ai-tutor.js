...
toolContent.addEventListener('click',async e=>{
 ...
 if(e.target.id==='makeImage'){
  ...
  try{const d=await api('/api/ai/image',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:document.getElementById('imagePrompt').value})});area.innerHTML=`<img class="generated-preview" src="${escapeHtml(d.previewUrl||d.url)}" alt="Generated image"><p><a class="btn" href="${escapeHtml(d.url)}" target="_blank" rel="noopener">⬇ Download image</a></p>`}catch(err){area.innerHTML='<p>'+escapeHtml(err.message)+'</p>'}finally{e.target.disabled=false}
 }
 if(e.target.id==='makeDocument'){
  ...
  try{const d=await api('/api/ai/document',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title:document.getElementById('docTitle').value,format:document.getElementById('docFormat').value,content:document.getElementById('docContent').value})});area.innerHTML=`<p>Document ready.</p><a class="btn" href="${escapeHtml(d.url)}" target="_blank" rel="noopener">⬇ Download ${escapeHtml(d.format.toUpperCase())}</a>`}catch(err){area.innerHTML='<p>'+escapeHtml(err.message)+'</p>'}finally{e.target.disabled=false}
 }
 if(e.target.id==='makeVideo'){
  ...
  try{const d=await api('/api/ai/video',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:document.getElementById('videoPrompt').value})});area.innerHTML=`<div id="videoProgress"><p>Video job: ${escapeHtml(d.id)}</p><p>Status: <span id="videoStatus">${escapeHtml(d.status)}</span> <span id="videoPercent">${Number(d.progress||0)}%</span></p></div>`;pollVideo(d.id)}catch(err){area.innerHTML='<p>'+escapeHtml(err.message)+'</p>'}finally{e.target.disabled=false}
 }
});
