...
function attachmentMarkup(a, messageId) {
  ...
  if (kind === 'audio') { const mediaUrl = `/api/chat/attachments/${encodeURIComponent(messageId)}/media/audio`; return `<div class="chat-audio-wrap"><audio class="chat-audio" controls preload="metadata"><source src="${mediaUrl}" type="audio/mpeg">Your browser could not play this chat audio.</audio></div>`; }
...
function showAttachmentPreview(file) {
  ...
  const isAudio = type.startsWith('audio/') || /^(mp3|wav|m4a|ogg|oga|opus|aac|flac|amr|webm|aif|aiff|caf|mka)$/.test(ext);
  const isVideo = type.startsWith('video/') || /^(mp4|mov|m4v|3gp|avi|mkv)$/.test(ext) || (ext === 'webm' && !type.startsWith('audio/'));
