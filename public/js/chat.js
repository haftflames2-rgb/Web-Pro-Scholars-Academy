...
function hideVoicePreview() {
  ...
  if (audio) { audio.pause(); audio.removeAttribute('src'); audio.load(); audio.onloadedmetadata = null; audio.oncanplay = null; audio.onerror = null; }
  if (pendingVoiceUrl) { URL.revokeObjectURL(pendingVoiceUrl); pendingVoiceUrl = null; }
  pendingVoiceFile = null;
  $('voicePreview').classList.add('hidden'); $('recordingPanel').classList.add('hidden');
...
function showVoicePreview(blob, mime) {
  if (pendingVoiceUrl) URL.revokeObjectURL(pendingVoiceUrl);
  pendingVoiceUrl = URL.createObjectURL(blob);
  pendingVoiceFile = new File([blob], `voice-message-${Date.now()}.${extensionForAudioMime(mime)}`, { type: mime || blob.type || 'audio/webm' });
  const audio = $('voicePreviewAudio');
  audio.pause(); audio.removeAttribute('src'); audio.load();
  audio.preload = 'auto';
  audio.onloadedmetadata = () => {
    const duration = Number.isFinite(audio.duration) ? audio.duration : 0;
    $('recordingStatus').textContent = duration > 0 ? `Voice message ready — preview it (${formatDuration(duration)}) or delete it before sending` : 'Voice message ready — tap play to preview, or delete it before sending';
  };
  audio.oncanplay = () => { $('recordingStatus').textContent = 'Voice message ready — tap play to preview, or delete it before sending'; };
  audio.onerror = () => { $('recordingStatus').textContent = 'Preview could not load. Please record again.'; };
...
async function finishPcmRecording() {
  ...
  showVoicePreview(blob, 'audio/wav');
}
async function startAudioRecording() {
  if (mediaRecorder && mediaRecorder.state !== 'inactive') { mediaRecorder.stop(); return; }
  if (audioProcessor) { await finishPcmRecording(); return; }
  ...
  try {
    ...
    recordingStartedAt = Date.now(); updateRecordingTimer(); showRecordingPanel();
    ...
    if (window.MediaRecorder && recordingMimeType) {
      recordedChunks = [];
      mediaRecorder = new MediaRecorder(recordingStream, { mimeType: recordingMimeType });
      mediaRecorder.ondataavailable = e => { if (e.data?.size) recordedChunks.push(e.data); };
      mediaRecorder.onerror = () => { $('recordingStatus').textContent = 'Recording failed. Please try again.'; };
      mediaRecorder.onstop = () => {
        ...
        recordedChunks = [];
        if (recordingStream) { recordingStream.getTracks().forEach(t => t.stop()); recordingStream = null; }
        mediaRecorder = null; clearInterval(recordingTimer); recordingTimer = null; recordingStartedAt = 0;
        $('recordAudioBtn').textContent = '🎙️'; $('recordAudioBtn').title = 'Record audio';
        if (blob.size) showVoicePreview(blob, actualType); else hideRecordingPanel();
      };
      mediaRecorder.start(250);
    } else {
      // Last-resort PCM/WAV path for older browsers without MediaRecorder.
      if (typeof AudioContext === 'undefined') throw new Error('No supported audio recorder');
    ...
    if (recordingStream) recordingStream.getTracks().forEach(t => t.stop()); recordingStream = null;
    if (audioContext) { try { await audioContext.close(); } catch {} audioContext = null; }
    mediaRecorder = null; audioSource = null; audioProcessor = null; hideRecordingPanel(); alert('Microphone access was not allowed or this browser cannot record audio. Please allow microphone access and try again.');
  }
}
