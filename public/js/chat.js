...
function attachmentMarkup(a, messageId) {
  ...
  if (kind === 'audio') {
    const mediaUrl = `/api/chat/attachments/${encodeURIComponent(messageId)}/media/audio`;
    return `<div class="chat-audio-wrap"><audio class="chat-audio" controls preload="metadata" playsinline><source src="${mediaUrl}" type="audio/mpeg">Your browser could not play this chat audio.</audio></div>`;
  }
