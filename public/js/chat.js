...
function setVoiceRecordingBlob(blob, filename) {
  pendingVoiceFile = new File([blob], filename, { type: blob.type || 'audio/wav' });
  ...
  previewAudio.src = pendingVoiceUrl; previewAudio.load();
  ...
  try {
    ...
    if (mime && window.MediaRecorder) {
      ...
      mediaRecorder.onstop = () => {
        ...
        clearInterval(recordingTimer); recordingTimer = null; recordingStartedAt = 0;
        $('recordAudioBtn').textContent = '🎙️'; $('recordAudioBtn').title = 'Record audio';
        if (blob.size) setVoiceRecordingBlob(blob, `voice-message-${Date.now()}.${ext}`); else hideRecordingPanel();
      };
