...
function attachmentMarkup(a, messageId) {
  ...
  if (a.kind === 'video') { const mediaUrl = `/api/chat/attachments/${encodeURIComponent(messageId)}/media/video`; return `<video class="chat-video" controls preload="metadata" playsinline style="width:100%;border-radius:10px"><source src="${mediaUrl}">Your browser could not play this chat video.</video>`; }
  if (a.kind === 'audio') { const mediaUrl = `/api/chat/attachments/${encodeURIComponent(messageId)}/media/audio`; return `<div class="chat-audio-wrap"><p><b>🎵 Audio</b></p><audio class="chat-audio" controls preload="metadata" style="width:100%"><source src="${mediaUrl}">Your browser could not play this chat audio.</audio></div>`; }
...
function showAttachmentPreview(file) {
  ...
  media.innerHTML = '';
  // Images can still be shown as a lightweight thumbnail. Audio/video are NOT
  // previewed before sending; this avoids the mobile playback/decoder issue.
  if (isImage) {
    ...
    media.innerHTML = `<div class="chat-selected-file"><span class="chat-selected-file-icon">${isVideo ? '🎥' : isAudio ? '🎵' : '📄'}</span><span>${isVideo ? 'Video ready to send' : isAudio ? 'Audio ready to send' : 'Document ready to send'}</span></div>`;
  }
