...
function showAttachmentPreview(file) {
  ...
  if (isImage) {
    ...
    const v = document.createElement('video'); v.src = pendingAttachmentUrl; v.controls = true; v.playsInline = true; v.preload = 'auto'; v.className = 'chat-attachment-preview-video'; media.appendChild(v); v.load();
  } else if (isAudio) {
    const a = document.createElement('audio'); a.src = pendingAttachmentUrl; a.controls = true; a.preload = 'auto'; a.className = 'chat-attachment-preview-audio'; media.appendChild(a); a.load();
  } else {
...
async function finishPcmRecording() {
  ...
  $('voicePreviewAudio').onloadedmetadata = () => { $('recordingStatus').textContent = 'Voice message ready — preview or delete it before sending'; };
  $('voicePreviewAudio').onerror = () => { $('recordingStatus').textContent = 'Preview could not load. You can delete and record again.'; };
