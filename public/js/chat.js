...
async function setVoiceRecordingBlob(blob, filename) {
  try {
    ...
    // Keep the browser's native recording format for the local preview. On
    // Android Chrome/Samsung Internet this is substantially more reliable
    // than converting WebM/Opus or MP4 through decodeAudioData first.
    // The server can create a browser playback copy after upload.
    ...
    previewAudio.onloadedmetadata = () => {
      ...
      $('recordingStatus').textContent = duration > 0
        ? `Voice message ready (${formatDuration(duration)}) — preview or Send to send it`
        : 'Voice message ready — tap ▶ to preview, or Send to send it';
    };
    previewAudio.oncanplay = () => {
      $('recordingStatus').textContent = 'Voice message ready — tap ▶ to preview, or Send to send it';
    };
    previewAudio.onerror = () => {
      $('recordingStatus').textContent = 'Preview failed in this browser. You can still press Send to upload the recording.';
    };
