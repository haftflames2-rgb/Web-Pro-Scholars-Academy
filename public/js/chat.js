...
function attachmentMarkup(a, messageId) {
  ...
  if (kind === 'audio') { const mediaUrl = `/api/chat/attachments/${encodeURIComponent(messageId)}/media/audio`; const sourceType = String(a.mime || '').toLowerCase().startsWith('audio/') ? String(a.mime).split(';')[0] : (String(a.name || '').toLowerCase().startsWith('voice-message-') ? 'audio/webm' : 'audio/mpeg'); return `<div class="chat-audio-wrap"><audio class="chat-audio" controls preload="auto"><source src="${mediaUrl}" type="${sourceType}">Your browser could not play this chat audio.</audio></div>`; }
...
async function sendAttachment(file, caption = '', isVoice = false) {
  ...
  const form = new FormData(); form.append('file', file); if (caption) form.append('text', caption); if (replyTarget?.id) form.append('replyToId', replyTarget.id); if (isVoice) form.append('isVoice', '1');
...
async function finishPcmRecording() {
  // The recorder now uses MediaRecorder/Opus so the sent voice note is small
  // and can start playing quickly on mobile. Do not convert it to WAV.
...
async function startAudioRecording() {
  ...
  try {
    ...
    mediaRecorder = mime ? new MediaRecorder(recordingStream, { mimeType: mime, audioBitsPerSecond: 32000 }) : new MediaRecorder(recordingStream);
    recordedChunks = [];
    mediaRecorder.ondataavailable = e => { if (e.data && e.data.size) recordedChunks.push(e.data); };
    mediaRecorder.onerror = e => console.error('Voice recorder error:', e);
    mediaRecorder.onstop = async () => {
      ...
      await setVoiceRecordingBlob(blob, `voice-message-${Date.now()}.${ext}`);
    };
    ...
    recordingStream = null; mediaRecorder = null; recordedChunks = [];
    clearInterval(recordingTimer); recordingTimer = null; recordingStartedAt = 0;
    alert(e.message || 'Microphone access failed.');
  }
}
