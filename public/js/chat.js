...
async function sendAttachment(file, caption = '', isVoice = false) {
  ...
  try {
    ...
    appendMessage(d.message); lastMessageId = d.message.id; $('messageInput').value = ''; if (isVoice) { pendingVoiceFile = null; hideVoicePreview(); } else { clearPendingAttachment(); } clearReplyTarget(); $('messageList').scrollTop = $('messageList').scrollHeight; await loadConversations();
  } catch (e) { alert(e.message); }
...
function showAttachmentPreview(file) {
  ...
  const isVideo = type.startsWith('video/') || /^(mp4|webm|mov|m4v|3gp|avi|mkv)$/.test(ext);
  const isAudio = type.startsWith('audio/') || /^(mp3|wav|m4a|ogg|oga|opus|aac|flac|amr|aif|aiff|caf|mka)$/.test(ext);
  pendingAttachmentFile = file;
  pendingAttachmentUrl = URL.createObjectURL(file);
  const media = $('attachmentPreviewMedia'); if (!media) return;
  $('attachmentPreviewName').textContent = file.name;
  $('attachmentPreviewMeta').textContent = `${isVideo ? 'Video' : isAudio ? 'Audio' : isImage ? 'Image' : 'DOCX'} • ${formatBytes(file.size)}`;
  media.innerHTML = '';
  if (isImage) {
    const img = document.createElement('img'); img.src = pendingAttachmentUrl; img.className = 'chat-attachment-preview-image'; img.alt = file.name; media.appendChild(img);
  } else if (isVideo) {
    const v = document.createElement('video'); v.controls = true; v.playsInline = true; v.preload = 'auto'; v.className = 'chat-attachment-preview-video'; v.setAttribute('controlsList', 'nodownload'); v.src = pendingAttachmentUrl; media.appendChild(v); v.load();
  } else if (isAudio) {
    const a = document.createElement('audio'); a.controls = true; a.preload = 'auto'; a.className = 'chat-attachment-preview-audio'; a.src = pendingAttachmentUrl; media.appendChild(a); a.load();
  } else {
    media.innerHTML = '<div class="chat-file-preview-icon">📄</div>';
  }
  $('attachmentPreview').classList.remove('hidden'); $('chatSend').disabled = false;
}
...
async function setVoiceRecordingBlob(blob, filename) {
  try {
    // Normalize browser MediaRecorder output to PCM WAV before showing the
    // preview. Android browsers can record WebM/Opus or MP4 variants that
    // sometimes expose a 0:00 local preview even though the recording exists.
    const wav = blob.type === 'audio/wav' ? blob : await audioBlobToWav(blob);
    pendingVoiceFile = new File([wav], filename.replace(/\.[^.]+$/, '.wav'), { type: 'audio/wav' });
    const dataUrl = await blobToDataUrl(wav);
    if (pendingVoiceUrl?.startsWith?.('blob:')) URL.revokeObjectURL(pendingVoiceUrl);
    pendingVoiceUrl = dataUrl;
    const previewAudio = $('voicePreviewAudio');
    previewAudio.pause(); previewAudio.removeAttribute('src'); previewAudio.load();
    previewAudio.preload = 'auto';
    previewAudio.onloadedmetadata = () => { $('recordingStatus').textContent = `Voice message ready (${formatDuration(previewAudio.duration)}) — preview or delete it before sending`; };
    previewAudio.oncanplay = () => { $('recordingStatus').textContent = 'Voice message ready — tap ▶ to preview, or Send to send it'; };
    previewAudio.onerror = () => { $('recordingStatus').textContent = 'Preview could not load. Please record again.'; };
    previewAudio.src = dataUrl; previewAudio.load();
    $('voicePreview').classList.remove('hidden'); $('recordingPanel').classList.remove('hidden'); $('chatSend').disabled = false;
  } catch (e) {
    console.error('Voice preview normalization failed:', e);
    pendingVoiceFile = null;
    hideRecordingPanel();
    alert('The voice recording could not be prepared for playback. Please record again.');
  }
}
...
async function startAudioRecording() {
  ...
  if (!navigator.mediaDevices?.getUserMedia) { alert('Audio recording is not supported by this browser. You can still attach an audio file.'); return; }
  try {
    ...
    if (mime && window.MediaRecorder) {
      ...
      mediaRecorder.onstop = async () => {
        ...
        clearInterval(recordingTimer); recordingTimer = null; recordingStartedAt = 0;
        $('recordAudioBtn').textContent = '🎙️'; $('recordAudioBtn').title = 'Record audio';
        if (blob.size) await setVoiceRecordingBlob(blob, `voice-message-${Date.now()}.${ext}`); else hideRecordingPanel();
      };
    ...
    if (typeof AudioContext === 'undefined') throw new Error('Audio recording is not supported.');
    ...
    const silentGain = audioContext.createGain(); silentGain.gain.value = 0; recordedPcmChunks = [];
    audioProcessor.onaudioprocess = e => { recordedPcmChunks.push(new Float32Array(e.inputBuffer.getChannelData(0))); };
    audioSource.connect(audioProcessor); audioProcessor.connect(silentGain); silentGain.connect(audioContext.destination);
    recordingTimer = setInterval(updateRecordingTimer, 250);
  } catch (e) {
    if (recordingStream) recordingStream.getTracks().forEach(t => t.stop()); recordingStream = null;
    mediaRecorder = null; audioSource = null; audioProcessor = null;
    if (audioContext) { try { await audioContext.close(); } catch {} audioContext = null; }
    clearInterval(recordingTimer); recordingTimer = null; hideRecordingPanel(); alert('Microphone access was not allowed. Please allow microphone access in your browser.');
  }
}
