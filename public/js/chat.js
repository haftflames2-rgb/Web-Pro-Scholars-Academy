...
function attachmentMarkup(a, messageId) {
  ...
  if (a.kind === 'video') { const mediaUrl = `/api/chat/attachments/${encodeURIComponent(messageId)}/media/video`; return `<video class="chat-video" controls preload="metadata" playsinline><source src="${mediaUrl}">Your browser could not play this chat video.</video>`; }
  if (a.kind === 'audio') { const mediaUrl = `/api/chat/attachments/${encodeURIComponent(messageId)}/media/audio`; return `<div class="chat-audio-wrap"><audio class="chat-audio" controls preload="metadata"><source src="${mediaUrl}">Your browser could not play this chat audio.</audio></div>`; }
